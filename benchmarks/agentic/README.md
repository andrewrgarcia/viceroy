...
This is a small, real, two-task benchmark with one clear, live, reproducible win (`dna-guard`) and one honestly documented capability ceiling (`dna-bound`), on a free local model. It is not the statistical weight ponytail's 12-task agentic run has: more tasks at `dna-guard`'s difficulty, more runs, more files would strengthen it further. The reader-cost axis (`cognitive-load.js`) is now format-aware (it was scoring qwen's correct-but-reformatted prose answers as high reader-cost, contradicting the task-done win; fixed, and pinned with qwen's real answer as a regression — see the top-level README's Round 8). All three axes — applyable, correct, low reader-cost — now agree on the same result.