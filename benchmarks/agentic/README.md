...
On `cache-fn` (the trivial task), a live `n=10` run with `qwen2.5-coder` scored **both arms at 100%** — the file is too short to discriminate. `dna-guard` exists because of that finding. Running `dna-guard` live then surfaced a second, sharper finding: a baseline answer can be 100% applyable while having silently deleted the target function and corrupted a neighbor — applyability alone doesn't catch this, which is why `blast-radius.js` exists (see the top-level README for the full story, including the model's actual captured output, preserved as `fixtures/qwen_baseline_dnaguard.txt`). `dna-bound` exists to test a *different* baseline failure: not destructive carelessness, but confident pattern-matching onto the wrong target — a no-op that looks plausible because three of four siblings really do share the copied guard.

## What it is and isn't

This is now a genuine (if small) multi-task benchmark: three tasks, three deterministic axes, a pooled cross-task aggregate that doesn't wash out which failure mode came from which task. It is not yet the full benchmark ponytail-scale numbers would need: more tasks, more files, more runs, ideally a second model alongside the free local one. This is the seed it grows from — but it's no longer a single task pretending to generalize.
